...
??? Proof "Důkaz korektnost Merge"
    ...
    Mějme dvě seřazené posloupnosti a pro spor předpokládejme, že po jejich slití dohromady algoritmem Merge nevznikla seřazená posloupnost $P$, tedy že exitují indexy $i, j$, takové že $i < j$ a $P[i] > P[j]$. Tedy $P[i]$ bylo do $P$ vloženo dřív než $P[j]$. Mohly nastat následující dva případy:
