...
??? Proof "Důkaz korektnost Merge"
    ...
    Mějme dvě seřazené posloupnosti a pro spor předpokládejme, že po jejich slití dohromady algoritmem Merge nevznikla seřazená posloupnost $P$. Merge vloží každý prvek z původních dvou posloupností do $P$ právě jednou, tedy jediný způsob, jak mohla taková situace nastat je, že existují indexy $i, j$, takové že $i < j$ a $P[i] > P[j]$. Tedy $P[i]$ bylo do $P$ vloženo dřív než $P[j]$. Mohly nastat následující dva případy:

      - $P[i]$ a $P[j]$ byly původně ve stejné posloupnosti. To je ale spor s tím, že tato posloupnost byla seřazená.
      - $P[i]$ a $P[j]$ byly původně v různých posloupnostech. Pak ale při vkládání $P[i]$ do výsledné posloupnosti $P$ musel být první prvek druhé posloupnosti menší než $P[i]$ a tedy se měl vložit on, nebo druhá posloupnost nebyla seřazená, tedy se opět dostáváme ke sporu.

??? Proof "Důkaz korektnosti MergeSortu" 
  
    Důkaz provedeme indukcí podle délky seřazené posloupnosti:
